...
if ssp != "No Data Assigned" :
    ...
    t.to_crs(epsg=projection, inplace=True)
    ...
    subprocess.run(["gdalwarp", "-te", str(bounds[0]), str(bounds[2]), str(bounds[1]), str(bounds[3]), dst1, raster_output_clip1,"-t_srs",src_crs])

    subprocess.run(["gdalwarp", "-te", str(bounds[0]), str(bounds[2]), str(bounds[1]), str(bounds[3]), dst2, raster_output_clip2,"-t_srs", src_crs])

    subprocess.run(["gdalwarp", "-te", str(bounds[0]), str(bounds[2]), str(bounds[1]), str(bounds[3]), dst3, raster_output_clip3, "-t_srs", src_crs])


    subprocess.run(['gdal_translate', '-a_srs', src_crs, raster_output_clip1, raster_output_clip1b])
    subprocess.run(['gdal_translate', '-a_srs', src_crs, raster_output_clip2, raster_output_clip2b])
    subprocess.run(['gdal_translate', '-a_srs', src_crs, raster_output_clip3, raster_output_clip3b])
